...
def select_all(x):
    ...
    return list(n.arange(x.shape[1]))
...
def select_nbest(x, y, clf, nbest, grp):
    ...
    da = n.mean(clf.fit(x, mf=True, grp=grp, n_jobs=1), 1)
    return grp2idx(grp, list(n.ravel(da.T).argsort()[-nbest:][::-1]))
