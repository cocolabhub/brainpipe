...
def classify(x, y, classifier='lda', kern='rbf', n_folds=10, rep=10, kind='sf', n_jobs=1, n_knn=3, n_perm=0, n_tree=100,
    ...
    if (type(classifier) is int) | (type(classifier) is str):
        clf = classifier_choice(
            classifier, kern=kern, n_knn=n_knn, n_tree=n_tree, priors=priors)
    else:
        clf = classifier

    # - Cross validation definition :
    if kind == 'mf' and n_perm == 0:  # Multi feature classification
        da, all_scores, cv_model = classify_fcn(
            x, y, clf, n_folds=n_folds, rep=rep, n_jobs=n_jobs, cvkind=cvkind)
    ...
    if n_perm == 0:
        ...
        cv_model = crossval_choice(
            y, cvkind=cvkind, n_folds=n_folds, rndstate=0)
    ...
    # list(pvalue[0])
    return 100*da, 100*all_scores, 100*permutation_scores, pvalue
...
def crossval_choice(y, cvkind='skfold', n_folds=10, rndstate=0, rep=10):
    y = n.ravel(y)
    if cvkind == 'skfold':
        cv_model = cross_validation.StratifiedKFold(
            y, n_folds=n_folds, shuffle=True, random_state=rndstate)
    elif cvkind == 'kfold':
        cv_model = cross_validation.KFold(
            len(y), n_folds=n_folds, shuffle=True, random_state=rndstate)
    elif cvkind == 'sss':
        cv_model = cross_validation.StratifiedShuffleSplit(
            y, n_iter=n_folds, test_size=1/n_folds, random_state=rndstate)
    elif cvkind == 'ss':
        cv_model = cross_validation.ShuffleSplit(
            len(y), n_iter=rep, test_size=1/n_folds, random_state=rndstate)
...
def classifier_comparison(x, y, clf, cvkind='skfold', n_folds=10):
    ...
    for k in range(0, nbclassifier):
        da[0, k] = n.mean(
            cross_validation.cross_val_score(clf[k], x, y, cv=cv_model))
...
# - Time generalization :
####################################################################
def timegeneralization(x, y, classifier=0, n_knn=10, n_tree=100, cvkind=None,
                       n_folds=10, rndstate=0, kern='rbf'):
    ...
    if cvkind is not None:
        cvmodel = crossval_choice(
            y, cvkind=cvkind, n_folds=n_folds, rndstate=rndstate)

    da = n.zeros([nbtime, nbtime])
    # Training dimension
    for k in range(0, nbtime):
        xx = checkfeat(x[:, k], y)
        # Tsting dimension
        for i in range(0, nbtime):
            xy = checkfeat(x[:, i], y)
            if k == i and cvkind is not None:
                da[i, k] = n.mean(
                    cross_validation.cross_val_score(clf, xx, y, cv=cvmodel))
            else:
                clf.fit(xx, y)
                pred = clf.predict(xy)
...
def optimal_tree_nb(x, y, treerange=range(64, 137, 8), n_folds=10, cvkind='skfold'):
    print('-> Optimized the number of trees. From',
          str(treerange[0]), 'to', str(treerange[-1]), 'step', str(treerange[1]-treerange[0]))
    ...
    for k in range(0, len(treerange)):
        ...
        treescores[:, k] = cross_validation.cross_val_score(
            clf, x, y, cv=cv_model)
...
def optimal_neighbor_nb(x, y, n_range=range(5, 100, 5), n_folds=10, cvkind='skfold'):
    print('-> Optimized the number of neighbor. From',
          str(n_range[0]), 'to', str(n_range[-1]), 'step', str(n_range[1]-n_range[0]))
    ...
    for k in range(0, lenrange):
        ...
        nscores[:, k] = cross_validation.cross_val_score(
            clf, x, y, cv=cv_model)
